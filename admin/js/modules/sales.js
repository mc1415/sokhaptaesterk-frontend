...
tableBody.addEventListener('click', async (e) => {
    if (e.target.classList.contains('view-details-btn')) {
        ...
        try {
            ...
            openDetailsModal(saleDetails);

        } catch (error) {
            modalContent.innerHTML = `<p class="error-cell">Error: ${error.message}</p>`;
        }
    }
});
...
function openDetailsModal(sale) {
    ...
    sale.sale_items.forEach(item => {
        itemsHtml += `<li>${item.quantity} x ${item.name_en} (@ $${parseFloat(item.price_at_sale).toFixed(2)})</li>`;
    });
    itemsHtml += '</ul>';
